...
export default function SchemeDetail() {
  ...
  return <main className="scheme-direct-detail">
    ...
    <section className="scheme-detail-grid">
      ...
      <aside className="scheme-detail-action"><h2>Verify before you apply.</h2><p>Scheme Sathi is a guide. Confirm current eligibility, documents, dates and application steps on the official portal.</p><a href={scheme.portalUrl} target="_blank" rel="noreferrer">Open official portal <ExternalLink size={15} /></a><button type="button" onClick={() => setSharePreviewOpen(true)}><Share2 size={15} /> Share scheme</button><button className="scheme-detail-whatsapp" type="button" onClick={() => setSharePreviewOpen(true)}>WhatsApp</button></aside>
    </section>
    {sharePreviewOpen && <SchemeSharePreviewCard scheme={scheme} initialLanguage="en" onClose={() => setSharePreviewOpen(false)} />}
  </main>;
}
