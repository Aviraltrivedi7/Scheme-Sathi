...
import {
  ...
  scoreScheme,
  states,
  toClientSchemeItem as toClientScheme,
  type Scheme,
  type UserProfile,
} from "@/lib/schemes";
...
// One observer for every [data-reveal] section on the current screen —
// elements fade up as they enter the viewport. Some embedded webviews never
// deliver IntersectionObserver callbacks, so a scroll/resize fallback marks
// anything already on screen; reduced-motion users see everything instantly.
function useReveal(deps: React.ReactNode) {
  useEffect(() => {
    ...
    // Failsafe: if no element was ever marked (observer callbacks dropped),
    // reveal everything rather than leave sections invisible.
    const t = window.setTimeout(() => {
      const anyVisible = nodes.some(node => node.classList.contains("is-visible"));
      if (!anyVisible) nodes.forEach(node => node.classList.add("is-visible"));
    }, 400);
    return () => {
...
function HomeScreen({
  ...
  return (
    <main>
        ...
        <div className="hero-copy">
          ...
          <h1>
            {text(
              language,
              <>
                Find the help that was{" "}
                <em className="hero-accent">made for you</em>.
              </>,
              <>
                आपके लिए <em className="hero-accent">बनी मदद</em> खोजें।
              </>
            )}
          </h1>
      ...
      <section className="section-block category-block" data-reveal>
...
function SchemeCard({
  ...
  return (
    <article
      ...
      tabIndex={0}
      role="button"
      aria-label={`${language === "hi" ? scheme.nameHindi : scheme.name} — ${language === "hi" ? "विवरण खोलें" : "open details"}`}
    >
      ...
      [...scored]
        .filter(
          scheme =>
            ...
            `${scheme.name} ${scheme.nameHindi} ${scheme.benefits} ${scheme.benefitsHindi} ${scheme.category} ${scheme.categoryHindi} ${scheme.administeringBody}`
              .toLowerCase()
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  try {
                    window.prompt(
                      text(language, "Copy this scheme link:", "यह योजना लिंक कॉपी करें:"),
                      url
                    );
                  } catch { /* prompt blocked — toast below still guides user */ }
                  toast.message(
                    text(
                      language,
                      "Copy was blocked — link shown for manual copy.",
                      "कॉपी ब्लॉक हुई — मैन्युअल कॉपी के लिए लिंक दिखाया गया।"
                    )
                  );
                }
              }}
...
export default function Home() {
  ...
  const [savedIds, setSavedIds] = useState<string[]>(() => {
    try {
      const raw = localStorage.getItem("scheme-saved");
  ...
  const matching = trpc.matching.run.useMutation({
    onSuccess: () => {
      ...
      } catch { /* private-mode: session-only profile */ }
      setScreen("results");
    },
    onError: () => {
      // Server unreachable/DB down: still show results using the identical
      // on-device scorer instead of stranding the user on the form.
      try {
        localStorage.setItem("scheme-profile", JSON.stringify(profile));
      ...
      toast.message(
        text(
          language,
          "Showing on-device matches — server refresh unavailable.",
          "डिवाइस पर मिलान दिखा रहे हैं — सर्वर रिफ्रेश उपलब्ध नहीं है।"
        )
      );
    },
  });
  ...
  useEffect(() => {
    ...
    } catch { /* storage full/blocked — app still works for this session */ }
  ...
  useEffect(() => {
    ...
    } catch { /* QuotaExceeded — offline snapshot below still best-effort */ }
  }, [savedIds]);
  useEffect(() => {
    try {
      ...
      /* Private-mode / quota failure: keep in-memory snapshot only */
      try {
        setOfflineSavedSnapshot(createOfflineSavedSchemesSnapshot(catalog, savedIds));
      } catch { /* never crash the home screen on storage failure */ }
    }
  }, [catalog, savedIds]);
  useEffect(() => {
