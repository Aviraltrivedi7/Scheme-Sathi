...
export default function Discover() {
  ...
  const [verificationStatus, setVerificationStatus] = useState<"all" | "officialDirectory" | "eligibilityVerified" | "officialNotice">("all");
  ...
  return (
    <main className="discover-page">
      ...
      <section className="discover-intro">
        ...
        <div className="discover-count"><strong>{catalog.isLoading ? "—" : schemes.length}</strong><span>{language === "hi" ? "इस सूची में योजनाएँ" : "schemes in this view"}</span></div>
      </section>
      <section className="discover-workspace">
        ...
        <section className="discover-results" aria-live="polite">
          ...
          {catalog.isError && <div className="desk-empty"><Search size={24} /><h3>{language === "hi" ? "योजना डेटा लोड नहीं हो सका।" : "Scheme data could not load."}</h3><p>{language === "hi" ? "कृपया कुछ समय बाद फिर कोशिश करें।" : "Please try again in a moment."}</p><button type="button" className="desk-secondary" onClick={() => catalog.refetch()}>{language === "hi" ? "फिर कोशिश करें" : "Retry"}</button></div>}
