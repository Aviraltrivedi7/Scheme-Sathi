...
import { createPilotDashboardArchiveBackup, createPilotDashboardArchiveRestorePreview, parsePilotDashboardArchiveBackup, type ParsedPilotDashboardArchiveBackup, type PilotDashboardArchiveRestorePreview } from "@/lib/pilotDashboardArchiveBackup";
...
import { AlertTriangle, Archive, ArchiveRestore, BarChart3, CalendarRange, CheckCircle2, Copy, Download, Eye, GripVertical, Inbox, Link2, Loader2, MoreHorizontal, Palette, Pencil, Pin, Plus, RotateCcw, Search, ShieldCheck, Trash2, Upload, UsersRound, XCircle } from "lucide-react";
import { toast } from "sonner";
import "./PilotAdmin.css";

type FeedbackStatus = "new" | "reviewed" | "followUp" | "archived";
type ArchiveImportPreviewState = {
  fileName: string;
  backup: ParsedPilotDashboardArchiveBackup;
  restorePreview: PilotDashboardArchiveRestorePreview;
};
...
function archiveRetentionLabel(archivedAt: number | null, retentionDays: number) {
  if (!archivedAt) return "Retention starts when this archive is updated.";
  const remaining = Math.max(0, Math.ceil((archivedAt + retentionDays * 24 * 60 * 60 * 1000 - Date.now()) / (24 * 60 * 60 * 1000)));
  return remaining === 0 ? "Scheduled for permanent deletion today." : `${remaining} day${remaining === 1 ? "" : "s"} left before permanent deletion.`;
}
...
export default function PilotAdmin() {
  ...
  const [archivedFolderQuickFilter, setArchivedFolderQuickFilter] = useState("");
  const [archivedColorQuickFilter, setArchivedColorQuickFilter] = useState<FolderColor | "">("");
  ...
  const [pdfMarginDraft, setPdfMarginDraft] = useState<"compact" | "standard" | "spacious">("standard");
  ...
  const [archiveImportPreview, setArchiveImportPreview] = useState<ArchiveImportPreviewState | null>(null);
  ...
  const archiveSettingsQuery = trpc.admin.pilot.views.archiveSettings.useQuery(undefined, { enabled: user?.role === "admin" });
  ...
  const deleteDashboardFolder = trpc.admin.pilot.views.deleteFolder.useMutation({
    onSuccess: async () => {
      setFolderFilter("");
      setFolderRenameDraft("");
      await utils.admin.pilot.views.list.invalidate();
      toast.success("Folder label removed. Saved views were kept.");
    },
    onError: error => toast.error(error.message),
  });
  const restoreArchivedDashboardViews = trpc.admin.pilot.views.restoreArchived.useMutation({
    onSuccess: async () => {
      setSelectedSavedViewIds([]);
      await utils.admin.pilot.views.list.invalidate();
      toast.success("Selected saved views restored.");
    },
    onError: error => toast.error(error.message),
  });
  ...
  const setArchiveRetention = trpc.admin.pilot.views.setArchiveSettings.useMutation({
    onSuccess: async () => {
      ...
      await utils.admin.pilot.views.list.invalidate();
      toast.success("Archive retention period updated.");
    },
    onError: error => toast.error(error.message),
  });
  ...
  const archiveRetentionDays = archiveSettingsQuery.data?.settings.retentionDays ?? 30;
  ...
  const archivedFolderCounts = useMemo(() => archivedSavedViews.reduce<Record<string, number>>((counts, view) => {
    if (view.folder) counts[view.folder] = (counts[view.folder] ?? 0) + 1;
    return counts;
  }, {}), [archivedSavedViews]);
  const archivedFolders = useMemo(() => Object.keys(archivedFolderCounts).sort((left, right) => left.localeCompare(right)), [archivedFolderCounts]);
  const archivedColorCounts = useMemo(() => archivedSavedViews.reduce<Record<FolderColor, number>>((counts, view) => {
    if (view.folderColor && folderColorOptions.some(option => option.value === view.folderColor)) {
      counts[view.folderColor as FolderColor] += 1;
    }
    return counts;
  }, { saffron: 0, marigold: 0, teal: 0, indigo: 0, plum: 0, slate: 0 }), [archivedSavedViews]);
  ...
  const visibleSavedViews = useMemo(() => {
    ...
    return scopedSavedViews.filter(view => {
      ...
      const folderMatches = savedViewScope === "archived" ? !archivedFolderQuickFilter || view.folder === archivedFolderQuickFilter : !folderFilter || view.folder === folderFilter;
      const colorMatches = savedViewScope !== "archived" || !archivedColorQuickFilter || view.folderColor === archivedColorQuickFilter;
      return searchMatches && folderMatches && colorMatches;
    });
  }, [archivedColorQuickFilter, archivedFolderQuickFilter, archivedViewSearch, folderFilter, savedViewScope, savedViewSearch, scopedSavedViews]);
  useEffect(() => {
  ...
  const exportArchivedDashboardViews = async () => {
    if (!archivedSavedViews.length) {
      toast.error("There are no archived saved views to back up.");
      return;
    }
    try {
      const backup = await createPilotDashboardArchiveBackup(archivedSavedViews);
      downloadTextFile(backup.contents, backup.fileName, "application/json;charset=utf-8");
      toast.success("Private archived-view backup downloaded with integrity metadata.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "The archived-view backup could not be created.");
    }
  };
  const importArchivedDashboardBackup = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const backup = await parsePilotDashboardArchiveBackup(await file.text());
      setArchiveImportPreview({
        fileName: file.name,
        backup,
        restorePreview: createPilotDashboardArchiveRestorePreview(backup.views, savedViews.map(view => view.name)),
      });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "The archived-view backup could not be read.");
    }
  };
  const confirmArchivedDashboardImport = () => {
    if (!archiveImportPreview) return;
    if (!archiveImportPreview.restorePreview.canRestore) {
      toast.error("Archive import would exceed your 20 saved-view limit.");
      return;
    }
    importArchivedDashboardViews.mutate({ views: archiveImportPreview.backup.views });
  };
    ...
    <DashboardLayout>
      <div className="pilot-admin">
        ...
        <section className="pilot-conversion-card" aria-label="Cohort conversion funnel">
          ...
          <Dialog open={Boolean(archiveImportPreview)} onOpenChange={open => { if (!open) setArchiveImportPreview(null); }}><DialogContent className="pilot-archive-import-dialog" showCloseButton><DialogHeader><DialogTitle>Restore archived backup</DialogTitle><DialogDescription>Review the local JSON backup before any private saved views are restored. The original file stays in this browser.</DialogDescription></DialogHeader>{archiveImportPreview && <div className="pilot-archive-import-preview"><div className="pilot-archive-import-file"><div><strong>{archiveImportPreview.fileName}</strong><small>{archiveImportPreview.restorePreview.importedViewCount} archived view{archiveImportPreview.restorePreview.importedViewCount === 1 ? "" : "s"} ready to restore</small></div><span className={`pilot-archive-integrity-badge ${archiveImportPreview.backup.integrity.status}`}><ShieldCheck size={13} /> Archive {archiveImportPreview.backup.integrity.formatVersion} · {archiveImportPreview.backup.integrity.status === "verified" ? "Integrity verified" : "Legacy file"}</span></div><dl className="pilot-archive-import-stats"><div><dt>Current saved views</dt><dd>{archiveImportPreview.restorePreview.existingViewCount} / 20</dd></div><div><dt>After restore</dt><dd>{archiveImportPreview.restorePreview.existingViewCount + archiveImportPreview.restorePreview.importedViewCount} / 20</dd></div><div><dt>Free slots after restore</dt><dd>{archiveImportPreview.restorePreview.remainingSlots}</dd></div><div><dt>Name conflicts</dt><dd>{archiveImportPreview.restorePreview.conflicts.length}</dd></div></dl>{archiveImportPreview.backup.integrity.status === "legacy" && <p className="pilot-archive-import-note"><AlertTriangle size={14} /> This older v1 backup has no integrity record. Its format and view fields were still validated locally.</p>}{!archiveImportPreview.restorePreview.canRestore && <p className="pilot-archive-import-warning"><AlertTriangle size={14} /> This restore would exceed your 20 saved-view limit. Remove views or choose a smaller backup, then try again.</p>}{archiveImportPreview.restorePreview.conflicts.length > 0 && <section className="pilot-archive-conflicts"><div><strong>Name conflicts</strong><small>Conflicting views receive a unique restored name; existing private views stay unchanged.</small></div><ul>{archiveImportPreview.restorePreview.conflicts.slice(0, 5).map(conflict => <li key={`${conflict.sourceName}-${conflict.restoredName}`}><span>{conflict.sourceName}</span><span>→</span><strong>{conflict.restoredName}</strong></li>)}</ul>{archiveImportPreview.restorePreview.conflicts.length > 5 && <small>+{archiveImportPreview.restorePreview.conflicts.length - 5} more conflict{archiveImportPreview.restorePreview.conflicts.length === 6 ? "" : "s"}</small>}</section>}<section className="pilot-archive-restore-list"><div><strong>Restore plan</strong><small>All imported views become Active and unpinned.</small></div><ul>{archiveImportPreview.restorePreview.views.map(view => <li key={`${view.name}-${view.restoredName}`}><span>{view.restoredName}</span>{view.folder && <span className={`pilot-folder-badge color-${view.folderColor ?? "saffron"}`}>{view.folder}</span>}</li>)}</ul></section></div>}<DialogFooter><button type="button" className="pilot-clear-report" disabled={importArchivedDashboardViews.isPending} onClick={() => setArchiveImportPreview(null)}>Cancel</button><button type="button" className="pilot-export-report" disabled={!archiveImportPreview?.restorePreview.canRestore || importArchivedDashboardViews.isPending} onClick={confirmArchivedDashboardImport}>{importArchivedDashboardViews.isPending ? <Loader2 className="spin" size={15} /> : <ArchiveRestore size={15} />} Confirm restore {archiveImportPreview?.restorePreview.importedViewCount ?? 0} view{archiveImportPreview?.restorePreview.importedViewCount === 1 ? "" : "s"}</button></DialogFooter></DialogContent></Dialog>
          <div className="pilot-saved-view-controls" aria-label="Saved dashboard views">
            ...
            <div className="pilot-view-filter-row">
              ...
              {savedViewScope === "active" && <label className="pilot-folder-filter">Folder<select value={folderFilter} onChange={event => setFolderFilter(event.target.value)}><option value="">All folders</option>{allFolders.map(folder => <option key={folder} value={folder}>{`${folder} (${folderCounts[folder]})`}</option>)}</select></label>}
            </div>
            {savedViewScope === "archived" && <div className="pilot-archive-quick-filters" aria-label="Archived-view quick filters"><div><strong>Archive quick filters</strong><small>Refine this private archived list by folder or color.</small></div><div className="pilot-archive-filter-groups"><div><span>Folders</span><div>{<button type="button" className={!archivedFolderQuickFilter ? "active" : ""} aria-pressed={!archivedFolderQuickFilter} onClick={() => setArchivedFolderQuickFilter("")}>All folders <b>{archivedSavedViews.length}</b></button>}{archivedFolders.map(folder => <button type="button" key={folder} className={archivedFolderQuickFilter === folder ? "active" : ""} aria-pressed={archivedFolderQuickFilter === folder} onClick={() => setArchivedFolderQuickFilter(folder)}>{folder} <b>{archivedFolderCounts[folder]}</b></button>)}</div></div><div><span>Colors</span><div>{<button type="button" className={!archivedColorQuickFilter ? "active" : ""} aria-pressed={!archivedColorQuickFilter} onClick={() => setArchivedColorQuickFilter("")}>All colors <b>{archivedSavedViews.length}</b></button>}{folderColorOptions.filter(color => archivedColorCounts[color.value] > 0).map(color => <button type="button" key={color.value} className={`color-${color.value} ${archivedColorQuickFilter === color.value ? "active" : ""}`} aria-pressed={archivedColorQuickFilter === color.value} onClick={() => setArchivedColorQuickFilter(color.value)}><i aria-hidden="true" />{color.label} <b>{archivedColorCounts[color.value]}</b></button>)}</div></div></div></div>}
            {savedViewScope === "active" && allFolders.length > 0 && <div className="pilot-folder-overview" aria-label="Saved view counts by folder">{allFolders.map(folder => <button type="button" key={folder} className={`${folderFilter === folder ? "active " : ""}color-${folderColors[folder] ?? "saffron"}`} onClick={() => setFolderFilter(folder)}><span>{folder}</span><strong className="pilot-folder-count-badge">{folderCounts[folder]}</strong></button>)}</div>}
            <div className="pilot-folder-color-legend" aria-label="Folder color legend"><strong>Folder color legend</strong>{folderColorOptions.map(color => <span key={color.value} className={`color-${color.value}`}><i aria-hidden="true" />{color.label}</span>)}</div>
            {savedViewScope === "active" && allFolders.length > 0 && <div className="pilot-folder-management"><div><strong>Folder quick actions</strong><small>{folderFilter ? `Managing private folder “${folderFilter}”.` : "Choose a folder above before using rename, color, or delete actions."}</small></div><DropdownMenu><DropdownMenuTrigger asChild><button type="button" className="pilot-folder-quick-trigger" disabled={!folderFilter} aria-label="Folder quick actions"><MoreHorizontal size={16} /> Actions</button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuLabel>{folderFilter || "Choose a folder"}</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem disabled={!folderFilter} onSelect={() => { setFolderRenameDraft(folderFilter); window.setTimeout(() => document.getElementById("pilot-folder-rename")?.focus(), 0); }}><Pencil size={14} /> Rename folder</DropdownMenuItem><DropdownMenuSub><DropdownMenuSubTrigger disabled={!folderFilter}><Palette size={14} /> Color label</DropdownMenuSubTrigger><DropdownMenuSubContent>{folderColorOptions.map(color => <DropdownMenuItem key={color.value} onSelect={() => folderFilter && setDashboardFolderColor.mutate({ folder: folderFilter, folderColor: color.value })}><span className={`pilot-menu-color-dot color-${color.value}`} aria-hidden="true" />{color.label}</DropdownMenuItem>)}</DropdownMenuSubContent></DropdownMenuSub><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" disabled={!folderFilter || deleteDashboardFolder.isPending} onSelect={() => { if (folderFilter && window.confirm(`Remove the “${folderFilter}” folder label? Saved views will be kept.`)) deleteDashboardFolder.mutate({ folder: folderFilter }); }}><Trash2 size={14} /> Delete folder label</DropdownMenuItem></DropdownMenuContent></DropdownMenu><label>New folder name<input id="pilot-folder-rename" value={folderRenameDraft} maxLength={40} onChange={event => setFolderRenameDraft(event.target.value)} placeholder="For example, Spring review" /></label><label className="pilot-folder-color-control"><span><Palette size={13} /> Color</span><select value={folderFilter ? folderColors[folderFilter] ?? "saffron" : "saffron"} disabled={!folderFilter || setDashboardFolderColor.isPending} onChange={event => folderFilter && setDashboardFolderColor.mutate({ folder: folderFilter, folderColor: event.target.value as FolderColor })}>{folderColorOptions.map(color => <option key={color.value} value={color.value}>{color.label}</option>)}</select></label><button type="button" className="pilot-folder-action" disabled={!folderFilter || !folderRenameDraft.trim() || folderRenameDraft.trim() === folderFilter || renameDashboardViewFolder.isPending} onClick={() => renameDashboardViewFolder.mutate({ fromFolder: folderFilter, toFolder: folderRenameDraft })}>Rename folder</button></div>}
            ...
            {visibleSavedViews.length > 0 && <div className="pilot-bulk-view-manager" aria-label="Bulk move saved views"><div className="pilot-bulk-heading"><div><strong>{savedViewScope === "archived" ? "Restore archived saved views" : "Bulk move saved views"}</strong><small>{selectedSavedViewIds.length ? `${selectedSavedViewIds.length} private view${selectedSavedViewIds.length === 1 ? "" : "s"} selected.` : savedViewScope === "archived" ? "Select one or more archived views to restore them to Active." : "Select one or more visible views, then choose a folder."}</small></div><div><button type="button" onClick={selectVisibleSavedViews}>Select shown</button><button type="button" disabled={!selectedSavedViewIds.length} onClick={() => setSelectedSavedViewIds([])}>Clear selection</button></div></div>{savedViewScope === "archived" && <div className="pilot-bulk-restore"><ArchiveRestore size={16} /><span>Restored views return to Active unpinned.</span><button type="button" className="pilot-folder-action" disabled={!selectedSavedViewIds.length || restoreArchivedDashboardViews.isPending} onClick={() => restoreArchivedDashboardViews.mutate({ viewIds: selectedSavedViewIds })}>Restore {selectedSavedViewIds.length || ""} view{selectedSavedViewIds.length === 1 ? "" : "s"}</button></div>}<div className="pilot-bulk-view-list">{visibleSavedViews.map(view => <label key={view.id}><input type="checkbox" checked={selectedSavedViewIds.includes(view.id)} onChange={event => toggleSavedViewSelection(view.id, event.target.checked)} /><span>{view.name}</span>{view.folder && <span className={`pilot-folder-badge color-${view.folderColor ?? "saffron"}`}>{view.folder}</span>}{view.isArchived && <small className="pilot-archive-retention"><Archive size={12} /> {archiveRetentionLabel(view.archivedAt, archiveRetentionDays)}</small>}</label>)}</div>{savedViewScope === "active" && <div className="pilot-bulk-move-action"><label>Move selected to folder<input value={bulkFolderDraft} maxLength={40} onChange={event => setBulkFolderDraft(event.target.value)} placeholder="Leave blank to clear folder" /></label><label className="pilot-view-folder-color">Color<select value={bulkFolderColorDraft} disabled={!bulkFolderDraft.trim()} onChange={event => setBulkFolderColorDraft(event.target.value as FolderColor)}>{folderColorOptions.map(color => <option key={color.value} value={color.value}>{color.label}</option>)}</select></label><button type="button" className="pilot-folder-action" disabled={!selectedSavedViewIds.length || moveDashboardViewsToFolder.isPending} onClick={() => moveDashboardViewsToFolder.mutate({ viewIds: selectedSavedViewIds, folder: bulkFolderDraft.trim() || null, folderColor: bulkFolderDraft.trim() ? bulkFolderColorDraft : null })}>Move {selectedSavedViewIds.length || ""} view{selectedSavedViewIds.length === 1 ? "" : "s"}</button></div>}</div>}
