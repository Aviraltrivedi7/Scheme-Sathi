...
function archiveRetentionLabel(archivedAt: number | null) {
  if (!archivedAt) return "Retention starts when this archive is updated.";
  const remaining = Math.max(0, Math.ceil((archivedAt + archiveRetentionDays * 24 * 60 * 60 * 1000 - Date.now()) / (24 * 60 * 60 * 1000)));
...
export default function PilotAdmin() {
  ...
  }), [funnelSummary, pdfCustomLogoUrlDraft, pdfDateFormatDraft, pdfFooterDraft, pdfHeaderDraft, pdfLogoDraft, pdfMarginDraft, quarterChange, reportEndDate, reportStartDate, trendCohortType, trendPeriod]);
