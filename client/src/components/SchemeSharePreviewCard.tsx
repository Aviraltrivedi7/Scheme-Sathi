...
import { createSchemeShareText, createSchemeShareUrl, createWhatsAppSchemeShareUrl, shareScheme } from "@/lib/schemeSharing";
...
export function SchemeSharePreviewCard({ scheme, initialLanguage, onClose }: { scheme: Scheme; initialLanguage: Language; onClose: () => void }) {
  ...
  const shareNatively = async () => {
    try {
      const result = await shareScheme(scheme, shareLanguage);
  ...
  const copyShare = async () => {
    try {
      setCopying(true);
      await navigator.clipboard.writeText(createSchemeShareText(scheme, shareLanguage));
