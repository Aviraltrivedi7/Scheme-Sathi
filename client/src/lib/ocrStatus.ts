...
export function getOcrBadge(status: OcrStatus, extraction: { confidence: OcrConfidence; concerns: string[] } | null, language: "en" | "hi", minimumConfidence: OcrConfidence = "medium") {
  const copy = (english: string, hindi: string) => language === "hi" ? hindi : english;
  const rank: Record<OcrConfidence, number> = { low: 1, medium: 2, high: 3 };
  const needsReview = status === "failed" || (status === "complete" && (!!extraction?.concerns.length || !!extraction && rank[extraction.confidence] < rank[minimumConfidence]));
