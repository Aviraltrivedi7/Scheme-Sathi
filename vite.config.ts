...
export default defineConfig({
  ...
  build: {
    ...
    emptyOutDir: true,
    // No manualChunks: hand-picking vendor groups (charts/data/ui/react-dom)
    // created chunk import cycles (recharts ↔ react-dom) that crash at module
    // evaluation with TDZ "Cannot access before initialization" — the page
    // rendered blank. Vite's default chunking plus React.lazy route splitting
    // (client/src/App.tsx) already cuts the entry from 1426KB to ~270KB, and
    // shared vendor deps get deduplicated into the common entry chunk.
  },
  server: {
    host: true,
    allowedHosts: [
