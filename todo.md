...
## Catalog automation (auto-sync for new schemes)

- [x] Review catalog seed flow, admin editor, Heartbeat/local-scheduler conventions, and drizzle migration history.
- [x] Add scheme_sources, scheme_sync_runs, pending_schemes, and scheme_sync_settings tables plus migration 0029.
- [x] Build source fetchers (manual JSON, data.gov.in CKAN, myScheme, RSS), strict normalization, content-hash diffing, and review-queue staging with per-source auto-publish defaulting to direct catalog publish.
- [x] Add admin schemeSync APIs (sources, sync now, runs, approve/reject), daily 04:00 UTC job with boot re-arm, and POST /api/scheduled/scheme-sync handler.
- [x] Build the Catalog Automation panel in Admin Schemes with source management, sync controls, review queue, and run history.
- [x] Test normalization, diffing, CKAN/RSS mapping, TypeScript, full suite (202 tests), production build, and live server boot.

## Direct-publish internet scan (no approval queue)

- [x] Research official 2025-26 launches via PIB/ministry portals and verify each portal link live.
- [x] Publish PM Dhan-Dhaanya Krishi Yojana, Pulse Self-Reliance Mission, GOBARdhan bioenergy scheme, and the domestic PNG incentive straight into the shared catalog.
- [x] Default all automation sources to auto-publish so future arrivals go straight to the catalog.

## 5-minute catalog sync bot

- [x] Add `*/n` step support to the standalone cron matcher with boundary tests.
- [x] Arm a 5-minute sync bot on every standalone boot (persisted task UID, delayed first sweep, no-database skip instead of failure).
- [x] Harden the sync callback with task-UID verification plus orphan/no-database skips and regression tests.
- [x] Expose admin worker status (bot live, next sweep, last sweep, watched sources) and switch the automation control to the 5-minute schedule.
- [x] Verify boot log, TypeScript, full suite (209 tests), production build, and live server health.

## Zero-install jugaad: portable MariaDB + PIB watcher bot

- [x] Ship portable MariaDB 11.4.9 under %LOCALAPPDATA% (no install/admin needed), repair duplicate statements in migrations 0023/0024, sync the drizzle journal, and migrate clean (exit 0).
- [x] Add a keyless `pib` source kind: listing poll, scheme-announcement keyword filter with event/space/defence blocklist, detail scrape, category inference, per-run caps, known-ID skip.
- [x] Prove end-to-end live: first sweep staged 4 real PIB announcements, junk rejected, tightened filter re-verified, re-sweep idempotent with zero duplicates.
- [x] Add one-click Start-SchemeSathi.ps1 (DB + app + browser).
- [x] Verify TypeScript, full suite (215 tests), production build, live DB health, and admin API flow.
