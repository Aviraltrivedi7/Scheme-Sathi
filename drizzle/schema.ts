...
export const pilotDashboardViews = mysqlTable(
  ...
  table => [
    ...
    index("pilot_dashboard_views_user_pinned_rank_updated_idx").on(
      table.userId,
      table.isPinned,
      table.pinnedRank,
      table.updatedAt
...
/**
 * Catalog automation: external scheme sources polled by the daily sync.
 * New arrivals land in `pending_schemes` for admin review, or straight into
 * the catalog when the source opts into auto-publish.
 */
export const schemeSources = mysqlTable(
  "scheme_sources",
  {
    id: varchar("id", { length: 64 }).primaryKey(),
    name: varchar("name", { length: 255 }).notNull(),
    kind: mysqlEnum("kind", ["manual", "datagov", "myscheme", "rss", "pib"]).notNull(),
...
export const schemeSyncRuns = mysqlTable(
  "scheme_sync_runs",
  {
    id: int("id").autoincrement().primaryKey(),
    sourceId: varchar("sourceId", { length: 64 }).notNull().references(() => schemeSources.id, { onDelete: "cascade" }),
  ...
  table => [
    index("scheme_sync_runs_source_idx").on(table.sourceId),
  ]
);

/** Staged external schemes awaiting admin approval before entering the catalog. */
export const pendingSchemes = mysqlTable(
  "pending_schemes",
  {
    id: int("id").autoincrement().primaryKey(),
    sourceId: varchar("sourceId", { length: 64 }).notNull().references(() => schemeSources.id, { onDelete: "cascade" }),
  ...
  table => [
    uniqueIndex("pending_schemes_source_external_idx").on(table.sourceId, table.externalId),
    index("pending_schemes_status_idx").on(table.status),
  ]
);

/** Durable owner row for the daily catalog-sync job (task UID + last run). */
export const schemeSyncSettings = mysqlTable(
  "scheme_sync_settings",
  {
    id: varchar("id", { length: 64 }).primaryKey(),
    scheduleCronTaskUid: varchar("scheduleCronTaskUid", {
      length: 65,
