...
vi.mock("./_core/heartbeat", () => ({
  // Standalone mode registers jobs even in dev (no platform deferral), so the
  // mock returns a well-formed job for due-date and snooze flows.
  createHeartbeatJob: vi.fn(async () => ({ taskUid: "mock-cron-task", nextExecutionAt: null })),
...
describe("document and admin routers", () => {
  ...
  it("keeps reviewer alert preferences private and saves due-date timing without creating a live scheduler in development", async () => {
    const preferences = { assignmentAlertsEnabled: false, dueDateRemindersEnabled: true, defaultReminderLeadHours: 36, maxActiveAssignments: 4 };
  ...
  it("keeps workload reviewer-scoped while owner escalation and reviewer snooze enforce their own authenticated paths", async () => {
    const workload = { summary: { active: 2, assigned: 1, inReview: 1, overdue: 1, dueSoon: 1, escalated: 1 }, assignments: [] };
    const overdue = [{ assignmentId: 14, documentId: 3, documentName: "Income certificate", schemeName: "Scholarship", reviewerName: "Reviewer", status: "inReview", dueAt: 1790000000000, escalationState: "normal", escalationNote: null, escalatedAt: null }];
    mocks.getMyDocumentReviewWorkload.mockResolvedValue(workload); mocks.listOwnerOverdueDocumentReviews.mockResolvedValue(overdue); mocks.setDocumentReviewEscalation.mockResolvedValue(undefined); mocks.snoozeDocumentReviewDueReminder.mockResolvedValue({ previousTaskUid: null });
    const caller = appRouter.createCaller(context("user"));
    const myWorkload = await caller.documents.reviewers.workload(); const ownerOverdue = await caller.documents.reviewers.overdue(); await caller.documents.reviewers.escalation({ assignmentId: 14, action: "escalate", note: "Please complete today" }); const snooze = await caller.documents.reviewers.snooze({ assignmentId: 14, snoozeUntil: Date.now() + 7_200_000 });
    expect(mocks.getMyDocumentReviewWorkload).toHaveBeenCalledWith(5); expect(mocks.listOwnerOverdueDocumentReviews).toHaveBeenCalledWith(5); expect(mocks.setDocumentReviewEscalation).toHaveBeenCalledWith(5, 14, { action: "escalate", note: "Please complete today" }); expect(mocks.snoozeDocumentReviewDueReminder).toHaveBeenCalledWith(5, 14, expect.any(Number)); expect(myWorkload).toEqual(workload); expect(ownerOverdue.reviews).toEqual(overdue); expect(snooze).toEqual(expect.objectContaining({ deferred: expect.any(Boolean) }));
  });

  it("keeps capacity preference and escalation templates scoped to the authenticated account", async () => {
    const templates = [{ id: 9, name: "Gentle follow-up", body: "Hello {reviewer}", createdAt: 1790000000000, updatedAt: 1790000000000 }];
    mocks.listDocumentReviewEscalationTemplates.mockResolvedValue(templates); mocks.saveDocumentReviewEscalationTemplate.mockResolvedValue(9); mocks.deleteDocumentReviewEscalationTemplate.mockResolvedValue(undefined);
    const caller = appRouter.createCaller(context("user"));
    const listed = await caller.documents.reviewers.escalationTemplates.list(); const saved = await caller.documents.reviewers.escalationTemplates.save({ name: "Gentle follow-up", body: "Hello {reviewer}" }); const removed = await caller.documents.reviewers.escalationTemplates.remove({ templateId: 9 });
    expect(mocks.listDocumentReviewEscalationTemplates).toHaveBeenCalledWith(5); expect(mocks.saveDocumentReviewEscalationTemplate).toHaveBeenCalledWith(5, { name: "Gentle follow-up", body: "Hello {reviewer}" }); expect(mocks.deleteDocumentReviewEscalationTemplate).toHaveBeenCalledWith(5, 9); expect(listed.templates).toEqual(templates); expect(saved).toEqual({ templateId: 9 }); expect(removed).toEqual({ removed: true });
  });
