...
describe("document and admin routers", () => {
  ...
  it("keeps workload reviewer-scoped while owner escalation and reviewer snooze enforce their own authenticated paths", async () => {
    ...
    expect(mocks.getMyDocumentReviewWorkload).toHaveBeenCalledWith(5); expect(mocks.listOwnerOverdueDocumentReviews).toHaveBeenCalledWith(5); expect(mocks.setDocumentReviewEscalation).toHaveBeenCalledWith(5, 14, { action: "escalate", note: "Please complete today" }); expect(mocks.snoozeDocumentReviewDueReminder).toHaveBeenCalledWith(5, 14, expect.any(Number)); expect(myWorkload).toEqual(workload); expect(ownerOverdue.reviews).toEqual(overdue); expect(snooze).toEqual({ scheduled: false, deferred: true });
  });
