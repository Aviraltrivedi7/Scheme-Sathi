...
export const hasStandaloneLLM = () =>
  STANDALONE_MODE && Boolean(LLM_BASE_URL && LLM_API_KEY);
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  if (model) {
    payload.model = model;
  } else if (STANDALONE_MODE && LLM_BASE_URL) {
    // Standalone OpenAI-compatible endpoints have no server-side default model.
...
export async function streamLLM(params: InvokeParams, signal?: AbortSignal): Promise<Response> {
  ...
  payload.model = params.model ?? (STANDALONE_MODE && LLM_BASE_URL ? LLM_MODEL : undefined);
  ...
  const response = await fetch(resolveApiUrl(), { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${resolveApiKey()}` }, body: JSON.stringify(payload), signal });
