...
import {
  createLocalHeartbeatJob,
  deleteLocalHeartbeatJob,
  updateLocalHeartbeatJob,
} from "./localScheduler";
import { ENV } from "./env";
...
export async function createHeartbeatJob(
  ...
  if (STANDALONE_MODE) {
    return createLocalHeartbeatJob({
      name: job.name,
      cron: job.cron,
      path: job.path,
    });
...
export async function updateHeartbeatJob(
  ...
  if (STANDALONE_MODE) {
    return updateLocalHeartbeatJob(taskUid, { enable: patch.enable });
  }
