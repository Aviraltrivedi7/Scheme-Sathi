...
describe("public scheme score and engagement UX", () => {
  it("explains matching and non-matching points from the same profile rules", () => {
    ...
    expect(breakdown.find((item) => item.key === "farmer")).toMatchObject({ points: 3, matched: true });
  ...
  it("wires native/WhatsApp sharing, offline favorites, bounded comparison, and mobile hero order into the public UI", () => {
    ...
    expect(offlineReminders).toContain("Recent snooze activity");
    ...
    expect(offlineCalendar).toContain("Export CSV");
    ...
    expect(sharePreview).toContain("Rename duplicates");
    expect(sharePreview).toContain("Skip duplicates");
