...
export async function listPilotCohortConversionStats(filters?: PilotCohortMonthlyTrendFilters) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const [invites, visits, signups, feedback] = await Promise.all([
    listPilotCohortInvites().then(rows => filters?.cohortType ? rows.filter(row => row.cohortType === filters.cohortType) : rows),
    db
...
/**
 * Idempotent catalog seed. It writes only stable reviewed catalog records,
 * never user-generated content. Memoized per process: seeding 44 rows on every
 * catalog read was pure overhead once the first read had succeeded (admin
 * edits live in DB rows the upsert never rewrites, so a cached seed is safe).
 */
let schemeCatalogSeeded = false;

export async function ensureSchemeCatalog() {
  const db = await getDb();
  if (!db) databaseUnavailable();
  if (schemeCatalogSeeded) return db;
...
export async function listSchemeCatalog(filters?: SchemeCatalogListFilters) {
  try {
    ...
    // Public discovery must never break: without a database (fresh local
    // run, DB restarting), serve the reviewed shared catalog in-memory.
    // Authenticated/admin writes still require the DB and keep throwing.
    console.warn("[Database] listSchemeCatalog fallback to shared catalog:", error instanceof Error ? error.message : String(error));
    return filterSchemeCatalog(seedCatalog, filters);
  }
}
...
export async function getSchemeCatalogFilterOptions() {
  try {
    ...
    console.warn("[Database] getSchemeCatalogFilterOptions fallback:", error instanceof Error ? error.message : String(error));
    return {
      administeringBodies: Array.from(
        new Set(seedCatalog.map(row => row.administeringBody).filter(Boolean))
      ).sort((a, b) => a.localeCompare(b)),
    };
  }
}
...
export async function getSchemeById(schemeId: string) {
  try {
    ...
    const rows = await db
      ...
      .limit(1);
    if (rows[0]) return mapScheme(rows[0]);
  } catch (error) {
    console.warn("[Database] getSchemeById fallback to shared catalog:", error instanceof Error ? error.message : String(error));
  }
  return seedCatalog.find(row => row.id === schemeId);
}

/* ---------- Catalog automation (scheme sources + sync runs + review queue) ---------- */

export type SchemeSourceInput = {
  id?: string;
  name: string;
  kind: "manual" | "datagov" | "myscheme" | "rss" | "pib";
  endpoint: string;
  autoPublish?: boolean;
};

const slugSourceId = (name: string) =>
  name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48) || "source";

/** Seed disabled source templates so the admin sees where to point each integration. */
export async function ensureDefaultSchemeSources() {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const { DEFAULT_SCHEME_SOURCES } = await import("./schemeSync");
  await db
    .insert(schemeSources)
    .values(
      DEFAULT_SCHEME_SOURCES.map(source => ({
        id: source.id,
        name: source.name,
        kind: source.kind,
...
export async function listSchemeSources() {
  const db = await getDb();
  if (!db) databaseUnavailable();
  await ensureDefaultSchemeSources();
  return db.select().from(schemeSources);
}

export async function saveSchemeSource(input: SchemeSourceInput) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const name = input.name.trim().slice(0, 255);
  const endpoint = input.endpoint.trim().slice(0, 1024);
  if (name.length < 3) throw new Error("Give the source a descriptive name.");
  if (!/^https?:\/\/.+\..+/.test(endpoint)) throw new Error("Endpoint must be an http(s) address.");
  if (input.id) {
    await db
      ...
      .where(eq(schemeSources.id, input.id));
    const rows = await db.select().from(schemeSources).where(eq(schemeSources.id, input.id)).limit(1);
    return rows[0];
  }
  const id = `${slugSourceId(name)}-${randomBytes(4).toString("hex")}`.slice(0, 64);
  ...
  const rows = await db.select().from(schemeSources).where(eq(schemeSources.id, id)).limit(1);
  return rows[0];
}

export async function setSchemeSourceEnabled(sourceId: string, enabled: boolean) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  await db
    .update(schemeSources)
...
export async function recordSchemeSyncRun(input: {
  sourceId: string;
  ...
  const db = await getDb();
  if (!db) databaseUnavailable();
  await db.insert(schemeSyncRuns).values({
  ...
  await db
    ...
    .set({
      ...
      lastError: input.error?.slice(0, 512) ?? null,
      updatedAt: new Date(),
    })
    .where(eq(schemeSources.id, input.sourceId));
}

export async function listSchemeSyncRuns(sourceId?: string, limit = 20) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const base = db.select().from(schemeSyncRuns);
  const rows = sourceId
    ? await base.where(eq(schemeSyncRuns.sourceId, sourceId)).orderBy(desc(schemeSyncRuns.id)).limit(Math.min(50, Math.max(1, limit)))
    : await base.orderBy(desc(schemeSyncRuns.id)).limit(Math.min(50, Math.max(1, limit)));
  return rows;
}

export async function listPendingSchemes(status: "pending" | "approved" | "rejected" | "all" = "pending", limit = 50) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const base = db.select().from(pendingSchemes);
  const rows = status === "all"
    ? await base.orderBy(desc(pendingSchemes.id)).limit(Math.min(100, Math.max(1, limit)))
    : await base.where(eq(pendingSchemes.status, status)).orderBy(desc(pendingSchemes.id)).limit(Math.min(100, Math.max(1, limit)));
  return rows;
}
...
const accentForCategory = (category: string): "saffron" | "emerald" | "coral" | "indigo" => {
  const lowered = category.toLowerCase();
  if (lowered.includes("health")) return "emerald";
  if (lowered.includes("agri") || lowered.includes("farm")) return "saffron";
  if (lowered.includes("women") || lowered.includes("hous")) return "coral";
  return "indigo";
};

async function publishDraftToCatalog(
  db: NonNullable<Awaited<ReturnType<typeof getDb>>>,
  draft: {
    externalId: string; name: string; nameHindi: string; category: string; categoryHindi: string;
    level: "Central" | "State"; administeringBody: string; benefits: string; benefitsHindi: string;
    eligibility: Record<string, unknown>; documents: string[]; documentsHindi: string[];
    steps: string[]; stepsHindi: string[]; portalUrl: string; sourceUrl: string | null;
    applicationDeadline: number | null; deadlineLabel: string | null;
  },
  sourceName: string,
  sourceId: string
) {
  const baseId = draft.externalId.toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "scheme";
  const existing = await db.select({ id: schemeCatalog.id }).from(schemeCatalog).where(eq(schemeCatalog.id, baseId)).limit(1);
  const catalogId = existing[0] ? `${baseId}-${sourceId}`.slice(0, 96) : baseId;
  const reviewed = `Auto-sync ${sourceName} — verify on official portal`;
  await db
    .insert(schemeCatalog)
    .values({
      id: catalogId,
      ...
      categoryHindi: draft.categoryHindi,
      level: draft.level,
      administeringBody: draft.administeringBody,
      benefits: draft.benefits,
      benefitsHindi: draft.benefitsHindi,
      eligibility: draft.eligibility as SchemeCatalogItem["eligibility"],
      documents: draft.documents.length ? draft.documents : ["Official scheme specification"],
      documentsHindi: draft.documentsHindi.length ? draft.documentsHindi : ["आधिकारिक योजना विवरण"],
      steps: draft.steps.length ? draft.steps : ["Verify requirements on the official portal before applying"],
      stepsHindi: draft.stepsHindi.length ? draft.stepsHindi : ["आवेदन से पहले आधिकारिक पोर्टल पर आवश्यकताएँ जाँचें"],
      portalUrl: draft.portalUrl,
      sourceUrl: draft.sourceUrl ?? draft.portalUrl,
      verificationStatus: "officialNotice",
      reviewed: reviewed.slice(0, 64),
      accent: accentForCategory(draft.category),
      artwork: artworkForCategory(draft.category),
      applicationDeadline: draft.applicationDeadline ? new Date(draft.applicationDeadline) : null,
...
/**
 * Run one source end-to-end: fetch → normalize → stage new/changed drafts,
 * optionally publishing straight to the catalog when the source opts in.
 */
export async function runSchemeSourceSync(sourceId: string, fetchFn: typeof fetch = fetch) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const { diffCatalog, draftContentHash, fetchSourceEntries, normalizeExternalScheme } = await import("./schemeSync");
  const sources = await db.select().from(schemeSources).where(eq(schemeSources.id, sourceId)).limit(1);
  const source = sources[0];
  if (!source) throw new Error("Scheme source not found");
  const startedAt = Date.now();
  try {
    const catalogRows = await db.select({ id: schemeCatalog.id }).from(schemeCatalog);
    const pendingRows = await db.select().from(pendingSchemes).where(eq(pendingSchemes.sourceId, source.id));
    const knownIds = new Set([
      ...catalogRows.map(row => row.id),
      ...pendingRows.map(row => row.externalId),
    ]);
    const entries = await fetchSourceEntries(
      { id: source.id, name: source.name, kind: source.kind as "manual" | "datagov" | "myscheme" | "rss" | "pib", endpoint: source.endpoint, enabled: source.enabled, autoPublish: source.autoPublish },
      fetchFn,
      knownIds
    );
    const drafts: { externalId: string; hash: string; payload: Record<string, unknown> }[] = [];
    let skipped = 0;
    for (const entry of entries.slice(0, 200)) {
      const normalized = normalizeExternalScheme(entry, source.name);
      if (!normalized.ok) { skipped += 1; continue; }
      const hash = draftContentHash(normalized.draft);
      drafts.push({ externalId: normalized.draft.externalId, hash, payload: normalized.draft as unknown as Record<string, unknown> });
    }
    const pendingByExternal = new Map(pendingRows.map(row => [row.externalId, row]));
    const diff = diffCatalog(
      [...catalogRows.map(row => ({ id: row.id, hash: "" })), ...pendingRows.filter(row => row.status === "pending").map(row => ({ id: row.externalId, hash: row.contentHash ?? "" }))],
      drafts
    );
    let newCount = 0;
    let updatedCount = 0;
    for (const externalId of [...diff.added, ...diff.changed]) {
      const draft = drafts.find(item => item.externalId === externalId);
      if (!draft) continue;
      // Already published under this id and unchanged there: skip (admin edits win).
      if (catalogRows.some(row => row.id === externalId) && diff.unchanged.includes(externalId)) continue;
      const prior = pendingByExternal.get(externalId);
      if (!prior) {
        if (source.autoPublish) {
          await publishDraftToCatalog(db, draft.payload as never, source.name, source.id);
          await db.insert(pendingSchemes).values({
            sourceId: source.id,
            externalId,
            ...
            status: "approved",
            reviewedAt: new Date(),
          }).onDuplicateKeyUpdate({ set: { payload: draft.payload, contentHash: draft.hash, status: "approved", reviewedAt: new Date() } });
        } else {
          await db.insert(pendingSchemes).values({
            sourceId: source.id,
            externalId,
            ...
            contentHash: draft.hash,
            status: "pending",
          }).onDuplicateKeyUpdate({ set: { payload: draft.payload, contentHash: draft.hash, fetchedAt: new Date() } });
        }
        newCount += 1;
      } else if (prior.status === "pending" && prior.contentHash !== draft.hash) {
        await db
          .update(pendingSchemes)
          .set({ payload: draft.payload, contentHash: draft.hash, fetchedAt: new Date() })
          .where(eq(pendingSchemes.id, prior.id));
        updatedCount += 1;
      }
    }
    await recordSchemeSyncRun({ sourceId: source.id, status: "success", fetchedCount: entries.length, newCount, updatedCount });
    return { sourceId: source.id, fetched: entries.length, skipped, newCount, updatedCount, durationMs: Date.now() - startedAt };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await recordSchemeSyncRun({ sourceId: source.id, status: "failed", error: message });
    throw new Error(`Sync failed for ${source.name}: ${message}`);
  }
}

export async function runAllEnabledSchemeSources(fetchFn: typeof fetch = fetch) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  await ensureDefaultSchemeSources();
  const sources = await db.select().from(schemeSources).where(eq(schemeSources.enabled, true));
  const outcomes: { sourceId: string; ok: boolean; message: string }[] = [];
  for (const source of sources.slice(0, 10)) {
    try {
      const result = await runSchemeSourceSync(source.id, fetchFn);
      outcomes.push({ sourceId: source.id, ok: true, message: `${result.newCount} new, ${result.updatedCount} updated of ${result.fetched} fetched` });
    } catch (error) {
      outcomes.push({ sourceId: source.id, ok: false, message: error instanceof Error ? error.message : String(error) });
    }
  }
  const setting = await db.select().from(schemeSyncSettings).where(eq(schemeSyncSettings.id, "daily")).limit(1);
  if (setting[0]) await db.update(schemeSyncSettings).set({ lastRunAt: new Date() }).where(eq(schemeSyncSettings.id, "daily"));
  else await db.insert(schemeSyncSettings).values({ id: "daily", lastRunAt: new Date() }).onDuplicateKeyUpdate({ set: { lastRunAt: new Date() } });
  return outcomes;
}

export async function approvePendingScheme(pendingId: number) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const rows = await db.select().from(pendingSchemes).where(eq(pendingSchemes.id, pendingId)).limit(1);
  const pending = rows[0];
  if (!pending) throw new Error("Pending scheme not found");
  if (pending.status === "approved") return { catalogId: null as string | null, already: true };
  const sources = await db.select().from(schemeSources).where(eq(schemeSources.id, pending.sourceId)).limit(1);
  const payload = pending.payload as unknown as Parameters<typeof publishDraftToCatalog>[1];
  const catalogId = await publishDraftToCatalog(db, payload, sources[0]?.name ?? "External source", pending.sourceId);
  await db.update(pendingSchemes).set({ status: "approved", reviewedAt: new Date() }).where(eq(pendingSchemes.id, pendingId));
  return { catalogId, already: false };
}

export async function rejectPendingScheme(pendingId: number) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  await db.update(pendingSchemes).set({ status: "rejected", reviewedAt: new Date() }).where(eq(pendingSchemes.id, pendingId));
}

export async function getSchemeSyncSetting() {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const rows = await db.select().from(schemeSyncSettings).where(eq(schemeSyncSettings.id, "daily")).limit(1);
  return rows[0] ?? null;
}

/** Cron dereference: the task UID is the only trusted lookup key for the sync job. */
export async function getSchemeSyncSettingByTaskUid(taskUid: string) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  const rows = await db.select().from(schemeSyncSettings).where(eq(schemeSyncSettings.scheduleCronTaskUid, taskUid)).limit(1);
  return rows[0] ?? null;
}

export async function saveSchemeSyncTask(taskUid: string) {
  const db = await getDb();
  if (!db) databaseUnavailable();
  await db
    .insert(schemeSyncSettings)
    .values({ id: "daily", scheduleCronTaskUid: taskUid })
    .onDuplicateKeyUpdate({ set: { scheduleCronTaskUid: taskUid, updatedAt: new Date() } });
  const rows = await db.select().from(schemeSyncSettings).where(eq(schemeSyncSettings.id, "daily")).limit(1);
  return rows[0];
}
...
export async function uploadApplicationDocument(
  ...
  const safeBase = safeStorageFileName(fileName);
  const uniqueSuffix = randomBytes(8).toString("hex");
  const checklistSlug = documentName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48) || "doc";
  const uploaded = await storagePut(
    `applications/${userId}/${trackedApplicationId}/${checklistSlug}_${uniqueSuffix}_${safeBase}`,
    bytes,
    mimeType
  );
  await db
