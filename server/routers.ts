...
const profileInput = z.object({
  ...
  occupation: z.string().trim().min(1).max(96),
...
export const appRouter = router({
  ...
  auth: router({
    ...
    mode: publicProcedure.query(() => ({
      mode: STANDALONE_MODE ? ("credentials" as const) : ("oauth" as const),
    })),
        ...
        z
          .object({
            ...
            verificationStatus: z
              .enum(["officialDirectory", "eligibilityVerified", "officialNotice"])
              .optional(),
            sort: z
      ...
      dueDates: router({
        set: protectedProcedure
          ...
          .mutation(async ({ ctx, input }) => {
            ...
            // Platform dev mode defers cron registration; standalone mode always
            // registers because the in-process local scheduler is the backend.
            if (process.env.NODE_ENV !== "production" && !STANDALONE_MODE)
  ...
  admin: router({
    ...
    schemeSync: router({
      sources: adminProcedure.query(async () => ({
        sources: await listSchemeSources(),
      })),
      saveSource: adminProcedure
        .input(
          z.object({
            ...
            kind: z.enum(["manual", "datagov", "myscheme", "rss", "pib"]),
            endpoint: z.string().trim().url().max(1024),
            autoPublish: z.boolean().optional(),
          })
        )
        .mutation(async ({ input }) => ({
          source: await saveSchemeSource(input),
        })),
      setEnabled: adminProcedure
        .input(z.object({ sourceId: z.string().min(1).max(64), enabled: z.boolean() }))
        .mutation(async ({ input }) => {
          await setSchemeSourceEnabled(input.sourceId, input.enabled);
          return { ok: true as const };
        }),
      triggerSync: adminProcedure
        .input(z.object({ sourceId: z.string().min(1).max(64).optional() }).optional())
        .mutation(async ({ input }) => {
          if (input?.sourceId) return { results: [await runSchemeSourceSync(input.sourceId)] };
          return { results: await runAllEnabledSchemeSources() };
        }),
      runs: adminProcedure
        .input(z.object({ sourceId: z.string().min(1).max(64).optional(), limit: z.number().int().min(1).max(50).optional() }).optional())
        .query(async ({ input }) => ({
          runs: await listSchemeSyncRuns(input?.sourceId, input?.limit ?? 20),
        })),
      pending: adminProcedure
        ...
        .query(async ({ input }) => ({
          pending: await listPendingSchemes(input?.status ?? "pending", input?.limit ?? 50),
        })),
      approvePending: adminProcedure
        .input(z.object({ pendingId: z.number().int().positive() }))
        .mutation(async ({ input }) => approvePendingScheme(input.pendingId)),
      rejectPending: adminProcedure
        .input(z.object({ pendingId: z.number().int().positive() }))
        .mutation(async ({ input }) => {
          await rejectPendingScheme(input.pendingId);
          return { ok: true as const };
        }),
      automationStatus: adminProcedure.query(async () => ({
        setting: await getSchemeSyncSetting(),
      })),
      automationEnable: adminProcedure.mutation(async ({ ctx }) => {
        const existing = await getSchemeSyncSetting();
        if (existing?.scheduleCronTaskUid)
          return { setting: existing, alreadyEnabled: true };
        const sessionToken =
          parseCookie(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
        const job = await createHeartbeatJob(
          {
            ...
            cron: "0 */5 * * * *",
            path: "/api/scheduled/scheme-sync",
            payload: {},
            description: "Scheme Sathi catalog sync bot: poll enabled scheme sources every 5 minutes",
          },
          sessionToken
        );
        return {
          setting: await saveSchemeSyncTask(job.taskUid),
          alreadyEnabled: false,
          nextExecutionAt: job.nextExecutionAt ?? null,
        };
      }),
      workerStatus: adminProcedure.query(async () => {
        let botLive = false;
        let nextExecutionAt: string | null = null;
        try {
          const { listLocalHeartbeatJobs } = await import("./_core/localScheduler");
          const job = listLocalHeartbeatJobs().find(
            entry => entry.name.startsWith("scheme-sathi-scheme-sync") && entry.enabled
          );
          if (job) {
            botLive = true;
            nextExecutionAt = job.nextExecutionAt;
          }
        } catch { /* platform mode: no in-process job list */ }
        let lastRunAt: number | null = null;
        let enabledSources = 0;
        try {
          const setting = await getSchemeSyncSetting();
          lastRunAt = setting?.lastRunAt?.getTime() ?? null;
          // Platform Heartbeat owns the schedule when a task UID is stored
          // but no local job exists.
          if (!botLive && setting?.scheduleCronTaskUid) botLive = true;
          const sources = await listSchemeSources();
          enabledSources = sources.filter(source => source.enabled).length;
        } catch { /* no database yet: the bot idles until MySQL is available */ }
        return { botLive, nextExecutionAt, lastRunAt, enabledSources };
      }),
    }),
    ocrPolicy: router({
