...
export const appRouter = router({
  ...
  documents: router({
    ...
    historyFilters: router({
      list: protectedProcedure.query(async ({ ctx }) => ({ filters: await listSavedVerificationHistoryFilters(ctx.user.id), shares: await listVerificationHistoryFilterShares(ctx.user.id), received: await listReceivedVerificationHistoryFilters(ctx.user.id) })),
