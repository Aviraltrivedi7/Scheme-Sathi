...
import { z } from "zod";
import { approveApplicationDocumentOcr, approveBatchDocumentOcr, assignDocumentReviewer, assignReminderHeartbeat, cancelApplicationReminder, createApplicationReminder, deleteDocumentPdfAnnotation, exportDocumentVerificationHistoryCsv, exportDocumentVerificationHistoryPdf, getApplicationDocumentPreview, getDocumentReminderSetting, getOcrPolicy, getSchemeById, getUserSchemeProfile, listDocumentExpiryNotifications, listDocumentPdfAnnotations, listDocumentReviewAssignments, listDocumentReviewAudit, listDocumentVerificationHistory, listFamilyFilterInvitationNotifications, listMyDocumentReviewAssignments, listReceivedVerificationHistoryFilterInvites, listReceivedVerificationHistoryFilters, listSavedSchemeIds, listVerificationHistoryFilterShares, listSavedVerificationHistoryFilters, listSchemeCatalog, listTrackedApplications, markDocumentExpiryNotificationRead, markFamilyFilterInvitationNotificationRead, removeApplicationDocument, removeSavedVerificationHistoryFilter, respondToVerificationHistoryFilterInvite, revokeDocumentReviewer, revokeVerificationHistoryFilterShare, runApplicationDocumentOcr, runBatchDocumentOcr, saveDocumentPdfAnnotation, saveDocumentReminderTask, saveUserSchemeProfile, saveVerificationHistoryFilter, setApplicationDocumentReviewState, setDefaultVerificationHistoryFilter, shareVerificationHistoryFilter, toggleSavedScheme, trackSchemeApplication, updateApplicationDocumentExpiry, updateMyDocumentReviewAssignment, updateOcrPolicy, updateSchemeAdmin, updateTrackedApplication, uploadApplicationDocument } from "./db";
...
export const appRouter = router({
  ...
  documents: router({
    ...
    reviewers: router({
      ...
      revoke: protectedProcedure.input(z.object({ assignmentId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { await revokeDocumentReviewer(ctx.user.id, input.assignmentId); return { revoked: true }; }),
      ...
      audit: protectedProcedure.input(z.object({ documentId: z.number().int().positive() })).query(async ({ ctx, input }) => ({ events: await listDocumentReviewAudit(ctx.user.id, input.documentId) })),
    }),
    annotations: router({
